...
def checker(sent_audio):
    # get the speech features of the audio 
    msg = get_features(sent_audio)
    # convert feature list into a dataframe and expand dataframe dimension to predict
    data = pd.DataFrame(msg)
    data = np.expand_dims(data, axis=0)
    # make the prediction after loading the trained model
    model = tf.keras.models.load_model("5_Emotion_Model_cnn_87.h5", compile=False)
    prediction= model.predict(data)
    # convert each prediction's probability into a percentage
    all_emotion=[str(100*np.max(n)) for n in prediction[0]]
    all_emotion=dict(enumerate(all_emotion))
    # correlate the names of the encoded emotions to each emotion
